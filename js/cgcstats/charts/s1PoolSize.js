...
loadData(){ 
  if (Object.keys(this.dataMP) > 1){
    ...
    xhr.open("GET", "CGCStats/hourS1PoolSize.txt", false); //This line is different in each Class as it references the Server Cache location for the reference data
    xhr.send();
    if (xhr.status==200){
...
chartConfig(){
  this.data = {
    ...
    datasets: [{
      label: 'Season 1 Farming Pool Size',
  ...
  let chartS1PoolSize = new ChartS1PoolSize('chartS1PS', 'buttonS1PS');