...
class ChartCGCUSD {
  ...
  loadData(){ 
    if (Object.keys(this.dataMP) > 1){
      ...
      xhr.open("GET", "CGCStats/hourCGCUSD.txt", false); //This line is different in each Class as it references the Server Cache location for the reference data
      xhr.send();
      if (xhr.status==200){
  ...
  chartConfig(){
    this.data = {
      ...
      datasets: [{
        label: 'Catgirl Coin price in $USD',
