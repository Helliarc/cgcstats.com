...
loadData(){ 
  if (Object.keys(this.dataMP) > 1){
    ...
    xhr.open("GET", "CGCStats/hourTotalFP.txt", false); //This line is different in each Class as it references the Server Cache location for the reference data
    xhr.send();
    if (xhr.status==200){
...
chartConfig(){
  this.data = {
    ...
    datasets: [{
      label: 'Season 1 Total Farming Power',
  ...
  let chartTotalFP = new ChartTotalFP('chartTFP', 'buttonTFP');